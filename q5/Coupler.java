...
//Making couples and storing them in Couple type array
		...
		for(i=0; i<6; i++){
			for(j=0; j<12; j++){
				...
				writer.println(boys[j].name+" is checking out "+girls[i].name);
				}
				...
				girls[i].attractiveness >= boys[j].min_att_requir ){
					...
					writer.println(boys[j].name+" checked out successfully and now is in relation with "+girls[i].name);
					break;
				}
			}
		}
...
//Compatibility printing and sorting issue
		System.out.println(k + " couples have been formed enter k less than " +k+ " for 5th part's ansr or 0 for nothing");
		int p;
